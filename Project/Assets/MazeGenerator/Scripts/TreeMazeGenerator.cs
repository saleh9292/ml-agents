...
public abstract class TreeMazeGenerator : BasicMazeGenerator {
	...
	private struct CellToVisit{
		public int Row;
		public int Column;
		public MAzeDirection MoveMade;

		public CellToVisit(int row, int column, MAzeDirection move){
			Row = row;
			Column = column;
			MoveMade = move;
		}
	...
	public override void GenerateMaze ()
	{
		...
		mCellsToVisit.Add (new CellToVisit (Random.Range (0, RowCount), Random.Range (0, ColumnCount),MAzeDirection.Start));
		...
		while (mCellsToVisit.Count > 0) {
			...
			if(ctv.Column+1 < ColumnCount && !GetMazeCell(ctv.Row,ctv.Column+1).IsVisited && !IsCellInList(ctv.Row,ctv.Column+1)){
				...
				movesAvailableCount++;
			}else if(!GetMazeCell(ctv.Row,ctv.Column).IsVisited && ctv.MoveMade != MAzeDirection.Left){
			...
			if(ctv.Row+1 < RowCount && !GetMazeCell(ctv.Row+1,ctv.Column).IsVisited && !IsCellInList(ctv.Row+1,ctv.Column)){
				...
				movesAvailableCount++;
			}else if(!GetMazeCell(ctv.Row,ctv.Column).IsVisited && ctv.MoveMade != MAzeDirection.Back){
			...
			if(ctv.Column > 0 && ctv.Column-1 >= 0 && !GetMazeCell(ctv.Row,ctv.Column-1).IsVisited && !IsCellInList(ctv.Row,ctv.Column-1)){
				...
				movesAvailableCount++;
			}else if(!GetMazeCell(ctv.Row,ctv.Column).IsVisited && ctv.MoveMade != MAzeDirection.Right){
			...
			if(ctv.Row > 0 && ctv.Row-1 >= 0 && !GetMazeCell(ctv.Row-1,ctv.Column).IsVisited && !IsCellInList(ctv.Row-1,ctv.Column)){
				...
				movesAvailableCount++;
			}else if(!GetMazeCell(ctv.Row,ctv.Column).IsVisited && ctv.MoveMade != MAzeDirection.Front){
			...
			if(movesAvailableCount > 0){
				...
				case MAzeDirection.Right:
					mCellsToVisit.Add(new CellToVisit(ctv.Row,ctv.Column+1,MAzeDirection.Right));
					break;
				case MAzeDirection.Front:
					mCellsToVisit.Add(new CellToVisit(ctv.Row+1,ctv.Column,MAzeDirection.Front));
					break;
				case MAzeDirection.Left:
					mCellsToVisit.Add(new CellToVisit(ctv.Row,ctv.Column-1,MAzeDirection.Left));
					break;
				case MAzeDirection.Back:
					mCellsToVisit.Add(new CellToVisit(ctv.Row-1,ctv.Column,MAzeDirection.Back));
					break;
				}
			}else{
